...
Web検索（WebSearch / WebFetch）とブラウザ自動操作（Claude in Chrome）を組み合わせ、前提知識調査（Ground Zero リサーチ）+ 5つの調査領域を段階的に実施する。

## リサーチの全体フロー

以下の6ステップを順番に実行する。各ステップの詳細はreferencesファイルを参照すること。
...
**Step 0（Ground Zero リサーチ）は全ての調査の前に必ず実行すること。** このステップで業界の基本構造、専門用語、ビジネスモデル、バリューチェーン、顧客構造、規制環境、市場の数字感覚、トレンドの方向性を先に把握することで、後続ステップの検索クエリの精度と調査の深さが格段に向上する。
...
全ステップをすべて終えてからまとめて出力してはならない。**各ステップの調査が完了するたびに、その場でファイルに書き出すこと。**
...
2. 調査完了後、直ちにファイルを作成し「0. 前提知識（Ground Zero リサーチ）」セクションを書き出す
3. ステップ1（市場調査）の調査を実施する
4. 調査完了後、直ちにファイルに「1. 市場調査」セクションを追記する
5. ステップ2（競合分析）の調査を実施する
6. 調査完了後、直ちにファイルに「2. 競合分析」セクションを追記する
7. 以降、各ステップ完了のたびに同様に追記する
8. 全ステップ完了後、「総括」と「参照ソース一覧」を末尾に追記する
...
ステップ0完了時にファイルを作成し、ヘッダーと最初のセクションを書き出す。以降のステップではセクションを追記していく。
